...
const predecessor = "77b2ad50e3b4191a535b3fac02badc2cfd2316f6";
test(
  "P008 fresh schema and exact 013-after-016 predecessor PostgreSQL upgrade",
  { timeout: 90000 },
  async () => {
    const name = "harbor-p008-upgrade-" + randomUUID();
    ...
    try {
      ...
      assert.ok(!paths.some((p) => p.endsWith("013_schedules.sql")));
      await old.query(
      ...
      assert.equal(
        Number(
          (await old.query("SELECT count(*) FROM schedules")).rows[0].count,
        ),
        0,
      );
