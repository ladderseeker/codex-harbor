...
- The cloud coding environment checked on 25 September 2026 was configured with HTTPS egress only and no SSH client, so it could not reach `harbor-vps`; network access is an environment setting the owner can change. From such an environment, VPS work goes through the manual [GitHub Actions deployment](docs/developer/github-actions-deploy.md) workflow. Its read-only `preflight` follows the existing authorization for read-only checks; `build` and `deploy` need the owner's go-ahead for that run.
- For diagnosis and deployment, inspect the actual installed configuration, services, and artifact identity, then follow the [personal VPS guide](docs/developer/personal-vps.md#use-and-recover) and [GitHub Actions deployment](docs/developer/github-actions-deploy.md) for this instance; [the managed deployment and SSH recovery documentation](docs/developer/deployment.md) covers the managed profile. Preserve the source/candidate/live boundaries above and follow existing user authorization; access as `root` does not authorize unrelated server changes. This handoff adds no separate approval gate and does not waive any deployment verification or release blocker.
