# The personal VPS has no off-host backup

- Severity: Medium; losing or damaging the host would lose all conversation history and Harbor state, with no copy elsewhere.
- Owner: Inbox; awaiting owner selection. No proposal covers it.
- Status: Open; from documentation and source inspection.
...
Impact: losing the VPS or its disk would lose every conversation, attachment and setting, and a failed migration could be recovered only from a checkpoint on the same host. The owner's project folders are outside Harbor's state and are not in these checkpoints either.
