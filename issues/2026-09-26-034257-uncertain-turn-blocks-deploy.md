...
- Severity: High; one uncertain turn stops every later deploy, and the personal profiles offer no way to clear it.
...
A correction must keep refusing while work can still run or a runtime is held, must not mark an uncertain turn complete or replay it, and must report uncertain operations as their own count. Because the workflow runs the candidate's own `deploy-release`, a corrected script applies to the deploy that installs it.
