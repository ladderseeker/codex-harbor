...
- **Fix deploy safety first.** A supervisor stop or crash restart can leave runtime membership unknown, which then blocks every later deploy and holds runtime capacity until a host reboot; the likely mechanism is identified below. Any uncertain turn, which a crash during a turn or one oversized reply can cause, also blocks every later deploy, because the personal profiles cannot clear it. The live instance also needs a look now: its read-only preflight on 25 September reported four retained runtimes, so a deploy at that moment would have refused, and if the default budget of four applies, new turns in other conversations would have waited.
...
| [Off-host backup issue](../../issues/2026-09-26-034258-personal-vps-off-host-backup.md) | New: the only deployed instance has no off-host backup. |
