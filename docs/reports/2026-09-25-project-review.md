...
- **Fix deploy safety first.** A supervisor stop or crash restart can leave runtime membership unknown, which then blocks every later deploy and holds runtime capacity until a host reboot; the likely mechanism is identified below. Any uncertain turn, which a crash during a turn or one oversized reply can cause, also blocks every later deploy, because nothing in the product clears it. The live instance also needs a look now: its read-only preflight on 25 September reported four retained runtimes, so a deploy at that moment would have refused, and if the default budget of four applies, new turns in other conversations would have waited.
...
Today's [deploy-release](../../infra/personal-vps/deploy-release) fails closed before it stops anything: promotion refuses while any active or uncertain operation, non-idle runtime or background mark exists, and every successful turn sets a background mark that stays until its runtime is released. A deploy therefore needs every retained runtime gone first. Idle runtimes retire about 30 minutes after their last turn; protected ones, which keep a background process such as a development server, stay until the owner uses **Stop processes**. The deploy path itself thus stops only an empty supervisor; the exposure to unknown membership is any other stop or crash restart while runtimes are retained, such as the stop made by the earlier promotion helper on 21 September.

Two other rules in `deploy-release` also block deploys. It counts uncertain operations as busy work, acknowledged or not, and an uncertain turn stays uncertain by design; in the personal profiles the API also refuses the recovery routes, so the conversation stays blocked. A single uncertain turn, left by a supervisor restart during a turn, a lost runtime connection, an unconfirmed cancellation or an oversized reply, therefore makes every later deploy refuse, with no recovery in the product; this change records it as a [new issue](../../issues/2026-09-26-034257-uncertain-turn-blocks-deploy.md). Separately, promotion renders the installed service files with the candidate's installer template, so a release that changes those files, as P025's own unit change does, is refused as drift.

The live instance shows the practical effect. The workflow's read-only preflight at 19:21 UTC on 25 September ([run 2](https://github.com/ladderseeker/codex-harbor/actions/runs/36179135848)) reported no busy operation, four non-idle runtimes, no idle runtime and four background marks. A deploy at that moment would have refused. If the instance uses the default budget of four, which the preflight does not report, those runtimes also filled it, so a turn in any other conversation would have waited, with "Waiting for runtime retirement to be confirmed." if any of them was unknown or retiring, and otherwise "Waiting for capacity held by active or protected resources.". The counts do not say whether those runtimes were protected, which **Stop processes** releases, or unknown, which Harbor releases only once a later check proves their processes gone and which need a reboot today when their cgroups no longer exist. The [deploy guide](../developer/github-actions-deploy.md#retained-runtimes-block-deploys) now documents this and how to tell the two apart. P025 fixes the stop, keeps uncertain turns and changed service files from blocking deploys, reports the counts separately and adds a startup proof that avoids the reboot.

Separately, Harbor makes no off-host backup of the only deployed instance: the checkpoints that promotion writes are same-host copies. Whether the provider or the owner keeps host-level snapshots was not checked. No proposal covers this yet; this change records it as an [issue](../../issues/2026-09-26-034258-personal-vps-off-host-backup.md).
...
3. **Capacity waits do not say who holds the capacity.** From source, when retained runtimes fill the budget, a new turn waits with "Waiting for capacity held by active or protected resources." or "Waiting for runtime retirement to be confirmed." without naming the conversations that hold it or offering to stop them. The only hint is the dot on those rows and its tooltip. The live preflight in [Shutdown and deploy safety](#shutdown-and-deploy-safety) suggests the owner may already meet this. P033.
...
5. **New conversations take their settings from whatever is open.** From source, New chat copies the model, effort and permission of the open conversation, even into another project, and after a fresh page load with no conversation open it starts from Read only and the default model and effort. The walkthrough did not test this. P030.
...
12. **Some controls do nothing.** Like and dislike under replies are local reactions that store nothing, and the owner's own messages carry decorative thumbs, which the interface design keeps as an owner-requested visual exception. P033 asks the owner whether to keep them.
...
Harbor defines four runtime profiles: managed, fixture, personal local and personal VPS. Only the personal VPS runs for the owner, and the owner's 21 September direction made Linux VPS the only required target. Measured at `d0c505b`:
...
| [GitHub Actions deployment](../developer/github-actions-deploy.md) | Replaces "has not yet run" with the 25 September run history; explains how retained runtimes, uncertain turns and changed service files block a deploy, how to clear retained runtimes and what run 2 reported; notes the absence of CI. |
...
| [AGENTS.md](../../AGENTS.md) | Notes that the checked cloud environment cannot use SSH, so VPS work goes through the deploy workflow, and that `build` and `deploy` need the owner's go-ahead; points VPS diagnosis to the personal VPS guides. |
...
| [Uncertain-turn issue](../../issues/2026-09-26-034257-uncertain-turn-blocks-deploy.md) | New: in the personal profiles an uncertain turn blocks every later deploy. |
| [Off-host backup issue](../../issues/2026-09-26-034258-personal-vps-off-host-backup.md) | New: Harbor makes no off-host backup of the only deployed instance. |
...
| --- | --- | --- | --- |
| Urgent | [P025](../../design/proposals/025-clean-supervisor-shutdown-and-restart.md) | Stops, restarts and deploys leave no unknown runtime membership, and uncertain turns stop blocking deploys | Every later change is deployed through this path; one bad stop or uncertain turn blocks all deploys |
...
| High | [P032](../../design/proposals/032-request-limits-per-client.md) | Internet clients cannot use up the owner's request or sign-in budget | Small, and removes an easy lockout of the owner |
...
Not yet planned: an off-host backup of the deployed instance, recorded as an [issue](../../issues/2026-09-26-034258-personal-vps-off-host-backup.md), and a way to continue an uncertain conversation in the personal VPS profile. Each would need its own proposal when the owner selects it.
...
- The live runtime counts come from one read-only preflight at 19:21 UTC on 25 September; the state may have changed since, the counts cannot distinguish protected from unknown runtimes, and the live runtime budget was not inspected.
- The off-host backup finding covers only what Harbor provides; host-level snapshots by the provider or the owner were not checked.
- The deploy blockage by uncertain turns comes from source. The preflight reported no busy operation, so the live instance was not in that state then.
...
- No application gate was rerun for this documentation change. Its gate is `node scripts/check-docs.mjs` and `git diff --check d0c505b`; the pull request records their results for its final revision.
