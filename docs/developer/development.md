...
History pagination uses a filter-bound cursor and a maximum page size of 50. The sidebar requests `order=updated` with optional `selectedId`: the eligible selected conversation comes first and counts within the first five, followed by descending durable `updated_at` and `id`. Selection itself never updates timestamps. Recent cursors bind order, selected ID and all filters and carry exact timestamp plus selected-row priority. A selection change invalidates old cursors/requests and reloads the retained depth from the beginning. Activity refresh replaces that depth and deduplicates IDs; recent traversal is not a snapshot. Default `order=created` and its existing cursors remain compatible, including search; `selectedId` is invalid with created order. Pinning never bypasses project, archive or search filters. Metadata edits require `expectedRevision`; changing archive visibility never cancels execution. Recovery requests capture `expectedGeneration`; a failed attempt is explicitly retried with its existing `recoveryId` and `expectedAttempt`. The old key always reconciles its original accepted attempt. A ready fence can be consumed once by a separately acknowledged new turn. The original operation remains uncertain, with an acknowledgement reference; it is not reclassified as successful or automatically replayed.
