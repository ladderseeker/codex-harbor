...
History pagination uses a filter-bound cursor and a maximum page size of 50. The sidebar requests `order=queried`, ordered by the latest durable user-message timestamp with session creation as the no-query fallback and ID descending as the tie-breaker. Queried cursors retain exact timestamp precision and bind all filters. Selection only highlights a row; it does not change order, cursor or loaded depth. Accepted input refreshes the retained depth and moves the conversation to its durable position, while drafts, metadata, assistant/tool output, completion and status changes do not. Default `order=created` and the existing `order=updated` mode/cursors remain compatible, including search; `selectedId` is valid only with updated order. Recent traversal is not a snapshot. Metadata edits require `expectedRevision`; changing archive visibility never cancels execution. Recovery requests capture `expectedGeneration`; a failed attempt is explicitly retried with its existing `recoveryId` and `expectedAttempt`. The old key always reconciles its original accepted attempt. A ready fence can be consumed once by a separately acknowledged new turn. The original operation remains uncertain, with an acknowledgement reference; it is not reclassified as successful or automatically replayed.
