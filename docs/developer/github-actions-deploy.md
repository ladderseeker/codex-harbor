...
`busyOperations` also counts uncertain operations. In this profile an uncertain turn cannot be cleared: the recovery routes are unavailable, and the conversation stays readable but blocked. Any uncertain turn therefore makes every `deploy` refuse, and **Stop processes** does not help. No supported recovery exists yet; the [uncertain-turn issue](../../issues/2026-09-26-034257-uncertain-turn-blocks-deploy.md) records it and [P025](../../design/proposals/025-clean-supervisor-shutdown-and-restart.md) proposes the fix.
