...
P015 is In progress. The [proposal](../../design/proposals/015-personal-vps-workspace.md) owns acceptance and the [decision](../../design/decisions/011-personal-vps-workspace.md) defines its personal trust model. Commands below exist in the current implementation; installed/account acceptance is still required before reporting a usable deployment.
...
Managed worktrees/copies, file-editor tools, attachments, standalone terminals, schedules, managed previews, API tokens and explicit uncertain-work recovery are unavailable in this profile. Personal attached previews use the configured separate-origin gateway. Conversation tools remain the file-editing path. Account settings explain SSH onboarding and show native readiness.
...
The later `/root` authorization is covered by renderer/admission contracts and reuses the unchanged exact-root browser/runtime behavior. Actual `/root` ACL provisioning and installed browser/account acceptance remain unverified; these checks do not inspect root-home contents or use its credentials.
...
The service retains successful native runtimes for 30 minutes, holds the workspace writer and allows same-conversation continuation. The owner stop action, expiry and graceful shutdown require confirmed group retirement before release. At most four contexts are active/retained. Failed retirement or abrupt restart clears the advertised background availability and preserves uncertainty and the writer fence. SSH inspection must establish retirement before any administrative recovery; never clear the fence just because the UI is disconnected.
