...
These ports are shared across this personal VPS account: verify that the configured endpoint serves the intended project. Attaching does not start a separate managed runner or establish project process ownership. Use **Stop background work** in the conversation to stop its owned server; closing the preview tab only closes the viewer. Managed preview controls described above belong to the separate managed installation profile.
