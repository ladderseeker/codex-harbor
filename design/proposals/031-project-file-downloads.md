...
- **P031-10:** A directory with more than 1,000 entries lists 1,000 with the truncation notice. A file over 1 MiB shows metadata and **Download** without a preview, and a file over 50 MiB is refused with the limit named. Filenames with quotes, semicolons and non-ASCII characters download under an RFC 6266 encoded name that the browser saves safely, and a name containing a control character is listed escaped and not served. On actual Linux, a path component swapped for a symbolic link between the check and the open is refused.
