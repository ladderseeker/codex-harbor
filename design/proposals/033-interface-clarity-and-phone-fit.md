...
**Owner decision at acceptance.** The interface design keeps like and dislike under replies as local-only reactions, and decorative thumbs under the owner's messages as an [owner-requested visual exception](../systems/006-interface.md). Neither stores anything. The default is to keep both unchanged. If the owner chooses removal, P033-09 covers it and the interface design, guide and prototype change with it.
...
- **Redirect.** Only `GET` requests whose `Accept` header explicitly lists `text/html`, and that target an application page, not `/api/`, `/auth/` or a built asset, receive `303 See Other` to the fixed `/auth/login`. A request without an `Accept` header, or with only `*/*`, keeps today's JSON 401, which `deploy-release`'s installed check and API clients expect. The target never comes from the request, and the return address stays `/`. Every other signed-out request keeps its current status and JSON error.
...
- **P033-03:** Every string on the reviewed list replaces its predecessor, and no tested screen shows the listed internal terms. In the personal VPS lane, an uncertain conversation shows the reviewed notice and **New conversation**, which opens a new conversation in the same project, and shows no settings-only composer. The recovery panel shows one input area after its first action, and the unknown-runtime text makes no promise about **Stop processes**.
- **P033-04:** A signed-out `GET /` and `GET /?conversation=<id>` that accept HTML receive `303` to `/auth/login`, whatever the request's host, query or headers. A signed-out `GET /` without an `Accept` header, or with `Accept: */*`, and a signed-out `GET /api/v1/me` still return the JSON 401, and a built asset keeps its current response. After sign-in the browser returns to `/`.
