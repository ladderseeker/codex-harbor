...
2. Signed-in owner requests are limited per server session. Unauthenticated requests are limited per client address, under a separate global ceiling that never consumes a session's budget.
...
This plan implements the architecture's existing forwarded-header rule and changes only how limit buckets are keyed. The existing error code, retry guidance and window lengths stay. The design gains a short statement of the three bucket kinds and of the trusted-proxy setting for each profile.
...
- **Keys.** A request counts as signed in only after its session is validated; it then uses a key derived from the session hash, never the raw cookie. Every other request, including one with an unknown or expired cookie, uses the client address, so random cookies cannot create new budgets. Sign-in starts use the client address plus a global counter.
...
- **P032-02:** 300 unauthenticated requests in 10 seconds from one client address cause no `429` for the signed-in owner, from another address or from the same one.
...
- **P032-05:** Filling the address map with distinct addresses leaves session buckets and the overflow bucket working, and memory stays bounded.
- **P032-06:** The personal VPS template renders the trusted-proxy setting and the deployment test checks it; without the setting, behavior matches today's.
- **P032-07:** `pnpm build`, `pnpm check`, `pnpm test`, P005's rate-limit browser scenarios and the personal VPS Linux lane pass.
- **P032-08:** With the owner's go-ahead, one bounded request through the real proxy route shows the forwarded client address used for limiting.

Gate: behavioral, with forged-header tests for the trust boundary. Fixture evidence cannot establish the live proxy's source address; P032-08 does.
