...
- **Distributed clients.** A client that controls many addresses or IPv6 /64 prefixes can still use up the global unauthenticated and sign-in ceilings and delay sign-in for a signed-out owner; signed-in requests are unaffected. Stopping that needs a firewall, which is excluded.
...
- **P032-06:** Filling the address map with distinct addresses leaves session buckets working, gives a new client address its own bucket, and keeps memory bounded. Requests from many addresses within one IPv6 /64 share one bucket.
