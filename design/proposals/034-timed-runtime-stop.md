...
- **API.** The conversation's `runtime.idleUntil` in the conversation snapshot and listings also carries the stop time of a protected runtime, and is null when no automatic stop applies. A timed stop records the same `background.stop-requested` event as **Stop processes**, with `reason: "background_timeout"`. The personal VPS profile has no API tokens, so no token route changes.
