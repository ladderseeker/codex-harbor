...
Accepted system contract · 7 September 2026; ownership reconciled 20 September 2026

This document owns architecture-wide invariants. The [subsystem designs](systems/README.md) define current feature contracts, source ownership and explicit partial/deferred boundaries; the [HTML guide](design-tokens.html) owns visual tokens and interaction patterns. Numbered decisions record material choices. The [workflow](workflow.md) governs bounded proposals and evidence-based completion under [D013](decisions/013-evidence-based-delivery-workflow.md). Design acceptance is not implementation or deployment evidence. Historical reviews are preserved in the [review record](#review-record).

## Recommendation and scope

The owner narrowed delivery on 13 September 2026 to the common-use scope in [D009](decisions/009-common-use-release.md), leaving self-development and managed extensions deferred. [D013](decisions/013-evidence-based-delivery-workflow.md) now governs selecting one cohesive outcome from current need and retires the old active-plan queue. Outstanding product/security/release gates remain with the [issue inbox](../issues/README.md); baseline reconciliation does not establish that they passed or require executing every old item before selecting a new need.
...
The sections below define shared system requirements, including explicitly deferred boundaries; they do not imply every capability is implemented. The [foundation report](../docs/reports/2026-09-07-p001-foundation.md) records deterministic application and actual Linux/XFS checks at its historical source. The subsystem designs and [developer guide](../docs/developer/development.md) distinguish current source from historical evidence and remaining live/recovery gates.
...
Publish supported, unsupported, and experimental capabilities with versioned evidence for the features delivered at each gate. The [conversations design](systems/001-conversations-and-access.md) identifies the runtime baseline; the [deployment design](systems/004-deployment-and-profiles.md) identifies profile and release/restore gates. Each selected change extends the relevant evidence, and runtime upgrades repeat applicable compatibility/isolation checks while preserving tested rollback packages and compatibility limits.
...
[D010](decisions/010-personal-local-experience.md) adds the explicitly selected personal local profile recorded historically in [P013](proposals/archive/013-personal-local-experience.md). That profile uses native host Codex execution and native account login with independent private state. [D011](decisions/011-personal-vps-workspace.md) separately authorizes the personal VPS profile recorded historically in [P015](proposals/archive/015-personal-vps-workspace.md), with HTTPS owner authentication, approved original folders, nonroot native execution and explicitly weaker isolation guarantees. [D012](decisions/012-personal-vps-development.md) extends that personal profile to full project development with explicit Git/network/temp capabilities and separate-origin attached previews. It retains loopback-only authenticated Harbor access and native sandbox policies, rejects unsupported capabilities, and cannot be enabled as a public/installed or fixture profile. The Linux runner, quota, credential and isolation requirements below remain the managed deployment contract; D011 is the explicit personal VPS exception. Personal local execution makes no equivalent Linux isolation claim. Current feature ownership and evidence limits live in the [deployment and profiles design](systems/004-deployment-and-profiles.md).
...
Run Linux isolation-sensitive execution inside the Linux environment. A convenient macOS process sandbox is not evidence that Linux deployment isolation passes. Bind development listeners to loopback, use local HTTPS and explicit local OIDC callbacks, and retain normal browser authentication/CSRF behavior. Caddy supports local HTTPS; development certificate trust is an environment setup step. Never disable certificate verification for real credential onboarding. The isolated fixture browser may use the fresh test instance's local certificate exception with disposable identities only. A local test identity provider exercises real OIDC; it is not an authentication bypass. Production startup rejects test-provider trust and protocol-fixture settings. [Caddy HTTPS](https://caddyserver.com/docs/automatic-https)

For host-run API services, select a fixed proxy template appropriate to the environment. Docker Desktop's proxy can reach the host through its supported host gateway. On native Linux, the trusted Caddy service may share the host network to reach the loopback-bound API; its local-development listener explicitly binds loopback because published-port restrictions do not apply to host networking. This authority belongs only to the trusted HTTPS proxy. Coding runners always retain the isolated D001 profile. Use this same selection in development and tests, and verify the actual listening interfaces. [Docker host networking](https://docs.docker.com/engine/network/drivers/host/), [Caddy listener binding](https://caddyserver.com/docs/caddyfile/directives/bind)
...
This table defines command semantics, not availability. Ordinary development, E2E, contract, live and isolation entry points exist in the current package manifest; `pnpm test:e2e:self` remains unavailable with the deferred self-development contract. New commands require implementation, never successful placeholders. A missing prerequisite must produce an explicit failure or documented unverified result, never a fabricated pass. Operational instructions for commands that actually exist belong in the [developer guide](../docs/developer/development.md).
...
[D007](decisions/007-confined-project-preview-origins.md) defines P011's preview boundary: a unique browser origin, current owner-session viewer grants, and a fixed relay joined only to an exact no-egress runner's loopback namespace. Default read-only previews retain D005's read-only checkout/common roots and a generation-bound reader lease; ordinary source writes may coexist, while conflicting project/workspace metadata operations account for that reader. Write-enabled previews use the shared exclusive writer. This does not broaden conversation/terminal grants or claim a firewall around the owner's browser.

[D001](decisions/001-confined-runtime-egress.md) makes the initial egress mechanism concrete: an isolated internal network for each runtime generation and a trusted HTTP model gateway that constructs the fixed upstream TLS connection. Generic CONNECT tunnels cannot enforce a hostname boundary on shared CDN addresses. Dispatch fails closed until the supported Linux profile is available; its current source and evidence boundaries are recorded in the [conversations design](systems/001-conversations-and-access.md).
...
[D002](decisions/002-registered-mount-authority.md) defines trusted mount ancestry, disjoint project registration, stored filesystem identity, and descriptor-relative operations inside project-controlled paths. This narrows the launcher's pathname trust boundary without exempting repository content from race-resistant access checks.

[D003](decisions/003-quota-backed-project-storage.md) defines the initial supported persistent-storage profile: trusted allocation from quota-backed XFS project slots, automatic private native-state directories, and real kernel enforcement checks. [D004](decisions/004-protected-runtime-credentials.md) defines supervisor-owned capability discovery and protected, encrypted runtime credential onboarding. Their local evidence is recorded in P001; authenticated live-account acceptance remains pending.
...
Store projects, workspaces, session/Codex identifiers, operations, events, approvals, attachments, schedules, credentials, and audits. Bound retention and storage growth. Back up the database, native runtime state, repositories, and attachments consistently. The [scheduling design](systems/003-scheduled-execution.md) defines local acceptance against delivered dependencies; relying on schedules in a managed installation also requires the [deployment design](systems/004-deployment-and-profiles.md)'s encrypted off-server backup and restore gate.
...
This is a deferred contract, with current availability and remaining acceptance owned by the [development and extensions design](systems/005-development-and-extensions.md). It is not a claim that the managed self-development workflow or its command exists on main.
...
[D008](decisions/008-isolated-self-development-workers.md) selects a fixed nonroot worker container with an emulated disposable Linux guest for the deferred self-development outcome. The candidate supervisor and guest Docker daemon can administer only that job's guest resources. Assume the guest and QEMU process may compromise the worker; the outer container's mount, namespace, capability, network and quota rules remain the host boundary. Trusted verification/report authority stays outside that entire worker, and tests the exact collected artifact in a fresh worker. Repository scripts and Dockerfiles are untrusted inputs and cannot redefine broker policy. Artifact retrieval uses registered IDs and digests, never arbitrary host paths. The complete outcome remains unavailable on main and requires integration and actual Linux acceptance; historical branch evidence does not establish current readiness.
...
[D006](decisions/006-portable-release-and-restore.md) specifies P009's immutable installed layout, actual trusted supervisor/storage privilege, encrypted backup checkpoint and module registry, fresh-host authority/filesystem rebinding, and SSH recovery. Its administrator tooling is implemented in part, with installed-profile evidence in the [deployment report](../docs/reports/2026-09-08-p009-development.md). Complete delivered-module recovery acceptance and the protected A-to-B restore gate remain incomplete; the [deployment design](systems/004-deployment-and-profiles.md) owns current scope and issue links. The design is not a deployment-readiness claim.
...
design/                   architecture, subsystem designs, workflow, decisions and proposals
docs/                     user/developer guides and verification reports
issues/                   concrete-problem inbox and archived history
```

## Delivery through complete feature outcomes

The [workflow](workflow.md) owns proposal states, dependency metadata, selection, file fences, delegation, review, evidence and record lifecycle. Use the [proposal template](proposal-template.md) for one bounded outcome against current designs. The [proposal](proposals/README.md) and [issue](../issues/README.md) directories provide discovery; their README files do not maintain a delivery queue.

Security and recovery gates belong to the outcome that changes their boundary. A feature must be independently exercisable after its explicit dependencies, without a future proposal making it testable. A selected change may reuse identified prior evidence for unchanged boundaries. Each subsequently delivered data-owning module extends backup, migration and release regression coverage; no fixture, design review or archive move establishes deployment readiness.

The [runtime compatibility issue](../issues/2026-09-07-073831-codex-runtime-compatibility.md) retains incomplete live, installed/reboot and release/restore evidence after legacy proposal retirement. The deferred self-development and extension outcomes have explicit unavailable contracts and inbox obligations. Main selects new work from current need under D013; these records do not authorize concurrent backlog execution.

## Review record

The earlier architecture review examined the designs for hostile-content rendering, cooperative writer admission, durable dispatch idempotency, and trusted control-plane separation. The expanded design subsequently completed two independent review rounds. Round 2 verified candidate credential scope, actual-Harbor-source self-development acceptance, accurate planned-delivery wording, and P001/P009 compatibility-gate ownership; no actionable design findings remained. See the [expanded design review report](../docs/reports/2026-09-07-design-review.md) for its historical scope, checks, and limitations. The [P001 foundation report](../docs/reports/2026-09-07-p001-foundation.md) subsequently recorded four implementation review rounds and bounded local results. D013 reconciles that historical plan into the archive without claiming its remaining live gates passed. Current requirements and evidence limits are recorded in the subsystem designs and inbox issues; local evidence does not establish deployment readiness.
