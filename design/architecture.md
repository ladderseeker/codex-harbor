...
Decision: Accepted · Delivery: In progress · 7 September 2026

The owner has authorized implementation of the roadmap and delegated implementation decisions. The technical choices below are the target contract; acceptance is not evidence of delivery. Material refinements are recorded in the linked decision documents. The expanded design completed two independent review rounds before implementation; see the [review record](#review-record). Decision status and delivery status remain independent, and the proposal index owns feature verification state.
...
The sections below define the complete target contract; they do not imply every feature is implemented. P001's local foundation has passed deterministic application and actual Linux/XFS checks, with live-account acceptance and medium follow-ups still open. The [foundation report](../docs/reports/2026-09-07-p001-foundation.md) and developer guide record actual evidence before portable VPS delivery.
...
[D001](decisions/001-confined-runtime-egress.md) makes the initial egress mechanism concrete: an isolated internal network for each runtime generation and a trusted HTTP model gateway that constructs the fixed upstream TLS connection. Generic CONNECT tunnels cannot enforce a hostname boundary on shared CDN addresses. Dispatch fails closed until the supported Linux profile is available; its implementation and isolation evidence belong to P001.
...
[D008](decisions/008-isolated-self-development-workers.md) selects a fixed nonroot worker container with an emulated disposable Linux guest for P010. The candidate supervisor and guest Docker daemon can administer only that job's guest resources. Assume the guest and QEMU process may compromise the worker; the outer container's mount, namespace, capability, network and quota rules remain the host boundary. Trusted verification/report authority stays outside that entire worker, and tests the exact collected artifact in a fresh worker. Repository scripts and Dockerfiles are untrusted inputs and cannot redefine broker policy. Artifact retrieval uses registered IDs and digests, never arbitrary host paths. This accepted contract awaits implementation and actual Linux evidence.
...
The earlier architecture review examined the designs for hostile-content rendering, cooperative writer admission, durable dispatch idempotency, and trusted control-plane separation. The expanded design subsequently completed two independent review rounds. Round 2 verified candidate credential scope, actual-Harbor-source self-development acceptance, accurate planned-delivery wording, and P001/P009 compatibility-gate ownership; no actionable design findings remained. See the [expanded design review report](../docs/reports/2026-09-07-design-review.md) for its historical scope, checks, and limitations. P001 later completed four implementation review rounds and is now Accepted and Implemented; the [proposal index](proposals/README.md) owns current delivery status. Its mandatory live and medium follow-up gates remain open; local evidence does not establish deployment readiness.
