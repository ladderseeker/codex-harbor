...
[D010](decisions/010-personal-local-experience.md) adds the explicitly selected personal local profile owned by [P013](proposals/013-personal-local-experience.md). Only that profile may use native host Codex execution and native account login with independent private state. It retains loopback-only authenticated Harbor access and native sandbox policies, rejects unsupported capabilities, and cannot be enabled as a public/installed or fixture profile. The Linux runner, quota, credential and isolation requirements below remain the deployment contract. Personal local execution makes no equivalent Linux isolation claim.
